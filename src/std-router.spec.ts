...
test("can runMatch", async ({ expect }) => {
	...
	const handler = vi.fn(({ render }) => render(42));
...
test("can bail early from middleware", async ({ expect }) => {
	const middlewareA = vi.fn((_, n) => n());
	const middlewareB = vi.fn(() => new Response("b"));
	const renderer = vi.fn((n, i) => new Response(n.toString(), i));
	const handler = vi.fn(({ render }) => render(42));
...
test("can access context in renderer", async ({ expect }) => {
	...
	const handler = vi.fn(({ render }) => render(1));
